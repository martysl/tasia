---
title: Custom Tools
description: Create tools the LLM can call in tasia.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside tasia's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.tasia/tools/` directory of your project.
- Or globally, by placing them in `~/.config/tasia/tools/`.
...
```ts title=".tasia/tools/database.ts" {1}
...
```ts title=".tasia/tools/project.ts" {8}
import { tool } from "@tasia-ai/plugin"
...
```python title=".tasia/tools/add.py"
import sys
...
```ts title=".tasia/tools/python-add.ts" {10}
import { tool } from "@tasia-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".tasia/tools/add.py")
