...
To debug issues with Tasia, start by checking the logs and local data it stores on disk.
...
- **macOS/Linux**: `~/.local/share/tasia/log/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\tasia\log`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `tasia --log-level DEBUG`.

---

## Storage

tasia stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/tasia/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\tasia`
...
Tasia Desktop runs a local Tasia server (the `tasia-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `Tasia` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.config/tasia/tasia.jsonc` (or `~/.config/tasia/tasia.json`)
- **macOS/Linux** (older installs): `~/.local/share/tasia/tasia.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\tasia\tasia.jsonc`
...
Tasia can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:

- **Global plugins**
  - **macOS/Linux**: `~/.config/tasia/plugins/`
  - **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\tasia\plugins`
- **Project plugins** (only if you use per-project config)
  - `<your-project>/.tasia/plugins/`
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so Tasia can rebuild it.

1. Quit Tasia Desktop completely.
2. Delete the cache directory:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/tasia`
- **Linux**: delete `~/.cache/tasia` (or run `rm -rf ~/.cache/tasia`)
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\tasia`

3. Restart Tasia Desktop.

---

### Fix server connection issues

Tasia Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If your `tasia.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.

#### Check environment variables

If you have `TASIA_PORT` set in your environment, the desktop app will try to use that port for the local server.

- Unset `TASIA_PORT` (or pick a free port) and restart.
...
On Windows, Tasia Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Notifications not showing

Tasia Desktop only shows system notifications when:

- notifications are enabled for Tasia in your OS settings, and
...
1. Quit Tasia Desktop.
2. Find and delete these files (they live in the Tasia Desktop app data directory):

- `tasia.settings.dat` (desktop default server URL)
- `tasia.global.dat` and `tasia.workspace.*.dat` (UI state like recent servers/projects)
...
If you're experiencing issues with Tasia:
...
3. Ensure you have the latest version with `tasia upgrade`
...
To figure out what models you have access to, run `tasia models`
...
2. If the issue persists, try clearing your stored configuration:
   ...
   ```bash
   rm -rf ~/.local/share/tasia
   ```

   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.local\share\tasia`
...
If you encounter API call errors, this may be due to outdated provider packages. tasia dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
1. Clear the provider package cache:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.cache\tasia`

2. Restart tasia to reinstall the latest provider packages

This will force tasia to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
tasia will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
