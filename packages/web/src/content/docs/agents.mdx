...
There are two types of agents in Tasia; primary agents and subagents.
...
Tasia comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Tasia comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

Tasia comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `tasia.json` config file:
...
- Global: `~/.config/tasia/agents/`
- Per-project: `.tasia/agents/`

```markdown title="~/.config/tasia/agents/review.md"
---
...
If no temperature is specified, Tasia uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Tasia config and the project specific config.
...
The model ID in your Tasia config uses the format `provider/model-id`. For example, if you're using [Tasia Zen](/docs/zen), you would use `tasia/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="tasia.json" {3-6,9-12}
{
  "$schema": "https://opencode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="tasia.json" {3-5,8-10}
{
  "$schema": "https://opencode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Run `tasia models` to see a list of the available models.
:::
...
```markdown title="~/.config/tasia/agents/docs-writer.md"
---
...
```markdown title="~/.config/tasia/agents/security-auditor.md"
---
