...
With Tasia you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Tasia uses our own `tasia` theme.
...
Tasia comes with several built-in themes.
...
- Want Tasia to match their terminal's appearance
...
Tasia supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/tasia/themes/*.json` or `$XDG_CONFIG_HOME/tasia/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.tasia/themes/*.json`
4. **Current working directory** - Defined in `./.tasia/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/tasia/themes
vim ~/.config/tasia/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .tasia/themes
vim .tasia/themes/my-theme.json
```
