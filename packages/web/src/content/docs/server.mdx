---
title: Server
description: Interact with tasia server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `tasia serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an tasia client can use.
...
```bash
tasia serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
tasia serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `TASIA_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `tasia`, or set `TASIA_SERVER_USERNAME` to override it. This applies to both `tasia serve` and `tasia web`.

```bash
TASIA_SERVER_PASSWORD=your-password tasia serve
```
...
When you run `tasia` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the tasia server to interact with tasia programmatically.
:::

This architecture lets tasia support multiple clients and allows you to interact with tasia programmatically.

You can run `tasia serve` to start a standalone server. If you have the
tasia TUI running, `tasia serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Tasia [IDE](/docs/ide) plugins.
...
The tasia server exposes the following APIs.
