...
You can add external tools to Tasia using the _Model Context Protocol_, or MCP. Tasia supports both local and remote servers.
...
You can define MCP servers in your [Tasia Config](https://opencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="tasia.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/tasia` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Tasia automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Tasia will:
...
If the server requires authentication, Tasia will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `tasia mcp auth <server-name>`.
...
```bash
tasia mcp auth my-oauth-server
```
...
```bash
tasia mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, Tasia will store the tokens securely in `~/.local/share/tasia/mcp-auth.json`.
...
tasia mcp debug my-oauth-server
```
...
Your MCPs are available as tools in Tasia, alongside built-in tools. So you can manage them through the Tasia config like any other tool.
...
```json title="tasia.json" {11, 14-18}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
tasia mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect Tasia to your Sentry account.
