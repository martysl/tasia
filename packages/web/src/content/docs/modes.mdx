...
:::caution
Modes are now configured through the `agent` option in the tasia config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in tasia allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the tasia config.
...
tasia comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.tasia/plans/*.md` to detail the plan itself
...
Configure modes in your `tasia.json` config file:
...
- Global: `~/.config/tasia/modes/`
- Project: `.tasia/modes/`

```markdown title="~/.config/tasia/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
both the global tasia config and the project specific config.
...
Create mode files in `.tasia/modes/` for project-specific modes or `~/.config/tasia/modes/` for global modes:

```markdown title=".tasia/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/tasia/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
