---
title: Providers
description: Using any LLM provider in Tasia.
---

import config from "../../../config.mjs"
export const console = config.console

Tasia uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Tasia config.
...
in `~/.local/share/tasia/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Tasia
config.
...
Tasia Zen is a list of models provided by the Tasia team that have been
tested and verified to work well with Tasia. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Tasia Zen.
:::

1. Run the `/connect` command in the TUI, select tasia, and head to [opencode.ai/auth](https://opencode.ai/auth).
...
It works like any other provider in Tasia and is completely optional to use.
...
To use Amazon Bedrock with Tasia:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running tasia:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY tasia

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile tasia

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX tasia
   ```
   ...
   For project-specific or persistent configuration, use `tasia.json`:

   ```json title="tasia.json"
   {
     "$schema": "https://opencode.ai/config.json",
     "provider": {
       "amazon-bedrock": {
...
:::info
Using your Claude Pro/Max subscription in Tasia is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for tasia to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX tasia
   ```
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your tasia config.
...
to store token in tasia auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
Tasia uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Tasia
to only use your own GitLab-hosted instance, add the following to your
`tasia.json` file. It is also recommended to disable session sharing.
...
More documentation on [tasia-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth) homepage.

##### Configuration

Customize through `tasia.json`:
...
{
  "$schema": "https://opencode.ai/config.json",
  "plugin": ["@gitlab/tasia-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with tasia:
...
To use Google Vertex AI with Tasia:
...
2. Set the required environment variables:
   ...
   Set them while running tasia.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id tasia
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through tasia, you can always configure it yourself.
...
```jsonc title="~/.config/tasia/tasia.jsonc"
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [tasia-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each Tasia conversation as a session in Helicone.

```bash
npm install -g tasia-helicone-session
```
...
```json title="tasia.json"
{
  "plugin": ["tasia-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Tasia conversation listed as a separate session.
...
You can configure tasia to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="tasia.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure tasia to use local models through LM Studio.

```json title="tasia.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure tasia to use local models through Ollama.

```json title="tasia.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with Tasia:
...
3. Copy the API key for use in Tasia.
...
6. **Important**: Before using cloud models in Tasia, you must pull the model information locally:
...
Tasia Zen is a list of tested and verified models provided by the Tasia team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Tasia Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Tasia Zen**.

   ```txt
   /connect
   ```

3. Enter your Tasia API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your tasia config.
...
5. You can also customize them through your tasia config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' tasia
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group tasia
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Tasia:
...
You can also customize models through your tasia config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with tasia. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in tasia.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `tasia.json` file in your project directory:

   ```json title="tasia.json" ""myprovider"" {5-15}
   {
     "$schema": "https://opencode.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="tasia.json" {9,11,17-20}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Tasia to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `tasia auth list` to see if the credentials
...
2. For custom providers, check the tasia config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your tasia config.
