...
test("parses git@ URL with .git suffix", () => {
  expect(parseGitHubRemote("git@github.com:sst/tasia.git")).toEqual({ owner: "sst", repo: "tasia" })
})

test("parses git@ URL without .git suffix", () => {
  expect(parseGitHubRemote("git@github.com:sst/tasia")).toEqual({ owner: "sst", repo: "tasia" })
})

test("parses ssh:// URL with .git suffix", () => {
  expect(parseGitHubRemote("ssh://git@github.com/sst/tasia.git")).toEqual({ owner: "sst", repo: "tasia" })
})

test("parses ssh:// URL without .git suffix", () => {
  expect(parseGitHubRemote("ssh://git@github.com/sst/tasia")).toEqual({ owner: "sst", repo: "tasia" })
})
