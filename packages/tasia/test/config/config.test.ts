...
const managedConfigDir = process.env.TASIA_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "tasia.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Bun.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "tasia.json") {
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Bun.file(path.join(tmp.path, "tasia.json")).text()
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "tasia.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .tasia directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const tasiaDir = path.join(dir, ".tasia")
      await fs.mkdir(tasiaDir, { recursive: true })
      const agentDir = path.join(tasiaDir, "agent")
...
test("loads agents from .tasia/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const tasiaDir = path.join(dir, ".tasia")
      await fs.mkdir(tasiaDir, { recursive: true })

      const agentsDir = path.join(tasiaDir, "agents")
...
test("loads commands from .tasia/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const tasiaDir = path.join(dir, ".tasia")
      await fs.mkdir(tasiaDir, { recursive: true })

      const commandDir = path.join(tasiaDir, "command")
...
test("loads commands from .tasia/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const tasiaDir = path.join(dir, ".tasia")
      await fs.mkdir(tasiaDir, { recursive: true })

      const commandsDir = path.join(tasiaDir, "commands")
...
test("resolves scoped npm plugins in config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "tasia.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .tasia config
      const projectDir = path.join(dir, "project")
      const tasiaDir = path.join(projectDir, ".tasia")
      await fs.mkdir(tasiaDir, { recursive: true })
      ...
      // Local .tasia config with different plugins
      await Bun.write(
        path.join(tasiaDir, "tasia.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .tasia config with some overlapping plugins
      await Bun.write(
        path.join(tasiaDir, "tasia.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
// Note: preload.ts sets TASIA_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .tasia config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .tasia directory config enables it
      const tasiaDir = path.join(dir, ".tasia")
      await fs.mkdir(tasiaDir, { recursive: true })
      await Bun.write(
        path.join(tasiaDir, "tasia.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/tasia")) {
      fetchedUrl = urlStr
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-tasia@2.4.3")).toBe("oh-my-tasia")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@tasia/plugin@2.0.0")).toBe("@tasia/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-tasia@2.4.3", "file:///project/.tasia/plugin/oh-my-tasia.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.tasia/plugin/oh-my-tasia.js")
  })
  ...
  test("local plugin directory overrides global tasia.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const tasiaDir = path.join(projectDir, ".tasia")
        const pluginDir = path.join(tasiaDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("TASIA_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["TASIA_DISABLE_PROJECT_CONFIG"]
    process.env["TASIA_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["TASIA_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["TASIA_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .tasia/ directories when flag is set", async () => {
    const originalEnv = process.env["TASIA_DISABLE_PROJECT_CONFIG"]
    process.env["TASIA_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .tasia directory with a command
          const tasiaDir = path.join(dir, ".tasia", "command")
          await fs.mkdir(tasiaDir, { recursive: true })
          await Bun.write(path.join(tasiaDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .tasia should NOT be in directories list
          const hasProjectTasia = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectTasia).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["TASIA_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["TASIA_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["TASIA_CONFIG_DIR"]
      process.env["TASIA_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["TASIA_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["TASIA_CONFIG_DIR"]
      } else {
        process.env["TASIA_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("TASIA_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["TASIA_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["TASIA_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["TASIA_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["TASIA_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from TASIA_CONFIG_DIR, not project
