...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    async tasia(input) {
      const hasKey = await (async () => {
        ...
        if (config.provider?.["tasia"]?.options?.apiKey) return true
        return false
      })()
    ...
    "amazon-bedrock": async () => {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from tasia.json provider config
    ...
    cerebras: async () => {
      return {
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "tasia",
          },
        },
      }
    },
  ...
  const state = Instance.state(async () => {
    ...
    for (const [providerID, provider] of Object.entries(providers)) {
      ...
      for (const [modelID, model] of Object.entries(provider.models)) {
        ...
        if (model.status === "alpha" && !Flag.TASIA_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
  ...
  export async function getSmallModel(providerID: string) {
    ...
    if (provider) {
      ...
      if (providerID.startsWith("tasia")) {
        priority = ["gpt-5-nano"]
      }
    ...
    // Check if tasia provider is available before using it
    const tasiaProvider = await state().then((state) => state.providers["tasia"])
    if (tasiaProvider && tasiaProvider.models["gpt-5-nano"]) {
      return getModel("tasia", "gpt-5-nano")
    }
