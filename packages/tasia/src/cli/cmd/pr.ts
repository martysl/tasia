...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run tasia",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for tasia session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found tasia session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`tasia import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting tasia...")
        UI.println()

        // Launch tasia TUI with session ID if available
        const { spawn } = await import("child_process")
        const tasiaArgs = sessionId ? ["-s", sessionId] : []
        const tasiaProcess = spawn("tasia", tasiaArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          tasiaProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`tasia exited with code ${code}`))
          })
          tasiaProcess.on("error", reject)
        })
      },
    })
  },
