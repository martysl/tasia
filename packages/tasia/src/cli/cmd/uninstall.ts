...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall tasia and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g tasia-ai",
      pnpm: "pnpm uninstall -g tasia-ai",
      bun: "bun remove -g tasia-ai",
      yarn: "yarn global remove tasia-ai",
      brew: "brew uninstall tasia",
      choco: "choco uninstall tasia",
      scoop: "scoop uninstall tasia",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall tasia -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using Tasia!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# tasia") || content.includes(".tasia/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".tasia/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".tasia/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".tasia"))
    ) {
      continue
    }
