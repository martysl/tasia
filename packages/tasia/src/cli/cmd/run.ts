...
import { createTasiaClient, type TasiaClient } from "@tasia-ai/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run tasia with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running tasia server (e.g., http://localhost:4096)",
      })
      .option("port", {
        type: "number",
  ...
  handler: async (args) => {
    ...
    const execute = async (sdk: TasiaClient, sessionID: string) => {
    ...
    if (args.attach) {
      const sdk = createTasiaClient({ baseUrl: args.attach })
      ...
      if (cfgResult.data && (cfgResult.data.share === "auto" || Flag.TASIA_AUTO_SHARE || args.share)) {
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createTasiaClient({ baseUrl: "http://tasia.internal", fetch: fetchFn })
