...
const AGENT_USERNAME = "tasia-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/tasia.yml"
          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                step2,
                "",
                "    3. Go to a GitHub issue and comment `/ta summarize` to see the agent in action",
                "",
...
jobs:
  tasia:
    if: |
      ...
      startsWith(github.event.comment.body, '/ta') ||
      contains(github.event.comment.body, ' /tasia') ||
      startsWith(github.event.comment.body, '/tasia')
    ...
    steps:
      ...
      - name: Run tasia
        uses: anomalyco/tasia/github@latest${envStr}
        with:
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/tasia,/ta")
          .split(",")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("tasia-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `tasia/${type}-${hex}-${timestamp}`
        }
        return `tasia/${type}${issueId}-${timestamp}`
      }
...
Co-authored-by: ${actor} <${actor}@users.noreply.github.com>"`
      ...
      function footer(opts?: { image?: boolean }) {
        ...
        const shareUrl = shareId ? `[tasia session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the tasia infrastructure after your response",
