...
import type { Event, TasiaClient, SessionMessageResponse } from "@tasia-ai/sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: TasiaClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `tasia auth login` in the terminal",
        name: "Login with tasia",
        id: "tasia-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // Tasia stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const tasiaProvider = providers.find((p) => p.id === "tasia")
    if (tasiaProvider) {
      if (tasiaProvider.models["big-pickle"]) {
        return { providerID: "tasia", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(tasiaProvider.models))
      if (best) {
        return {
