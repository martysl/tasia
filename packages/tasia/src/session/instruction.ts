...
function globalFiles() {
  ...
  if (!Flag.TASIA_DISABLE_CLAUDE_CODE_PROMPT) {
    files.push(path.join(os.homedir(), ".claude", "CLAUDE.md"))
  }
  if (Flag.TASIA_CONFIG_DIR) {
    files.push(path.join(Flag.TASIA_CONFIG_DIR, "AGENTS.md"))
  }
  return files
}
...
async function resolveRelative(instruction: string): Promise<string[]> {
  ...
  if (!Flag.TASIA_CONFIG_DIR) {
    log.warn(
      `Skipping relative instruction "${instruction}" - no TASIA_CONFIG_DIR set while project config is disabled`,
    )
    return []
  }
  return Filesystem.globUp(instruction, Flag.TASIA_CONFIG_DIR, Flag.TASIA_CONFIG_DIR).catch(() => [])
}
