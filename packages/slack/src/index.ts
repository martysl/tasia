...
import { createTasia, type ToolPart } from "@tasia-ai/sdk"
...
console.log("🚀 Starting tasia server...")
const tasia = await createTasia({
  port: 0,
})
console.log("✅ Tasia server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await tasia.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new tasia session...")
    const { client, server } = tasia
    ...
    console.log("✅ Created tasia session:", createResult.data.id)
  ...
  console.log("📝 Sending to tasia:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Tasia response:", JSON.stringify(result, null, 2))
