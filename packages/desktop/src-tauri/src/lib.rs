...
const SETTINGS_STORE: &str = "tasia.settings.dat";
...
fn get_sidecar_port() -> u32 {
    option_env!("TASIA_PORT")
        .map(|s| s.to_string())
        .or_else(|| std::env::var("TASIA_PORT").ok())
...
fn spawn_sidecar(app: &AppHandle, hostname: &str, port: u32, password: &str) -> CommandChild {
    ...
    .env("TASIA_SERVER_USERNAME", "tasia")
    .env("TASIA_SERVER_PASSWORD", password)
    .spawn()
    .expect("Failed to spawn tasia");
...
async fn check_server_health(url: &str, password: Option<&str>) -> bool {
    ...
    if let Some(password) = password {
        req = req.basic_auth("tasia", Some(password));
    }
        ...
        .setup(move |app| {
            ...
            let window_builder = WebviewWindowBuilder::from_config(&app, config)
                ...
                .initialization_script(format!(
                    r#"
                      window.__TASIA__ ??= {{}};
                      window.__TASIA__.updaterEnabled = {updater_enabled};
                    "#
                ));
...
async fn spawn_local_server(
    ...
    loop {
        if timestamp.elapsed() > Duration::from_secs(30) {
            break Err(format!(
                "Failed to spawn Tasia Server. Logs:\n{}",
