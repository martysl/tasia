...
import { createTasiaClient } from "@tasia-ai/sdk"
...
const { client, server } = createTasia()
...
try {
  ...
  console.log("tasia session", session.id)
  if (shareId) {
...
function createTasia() {
  const host = "127.0.0.1"
  const port = 4096
  const url = `http://${host}:${port}`
  const proc = spawn(`tasia`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createTasiaClient({ baseUrl: url })
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/tasia|\/ta)(?=$|\s)/)) {
    throw new Error("Comments must mention `/tasia` or `/ta`")
  }
}
...
async function assertTasiaConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to tasia server")
  }
}
...
async function getAccessToken() {
  ...
  if (isMock()) {
    ...
    const oidcToken = await core.getIDToken("tasia-github-action")
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/tasia" || body === "/ta") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/tasia") || body.includes("/ta")) {
      if (reviewContext) {
...
async function chat(text: string, files: PromptFiles = []) {
  console.log("Sending message to tasia...")
...
async function configureGit(appToken: string) {
  ...
  await $`git config --global user.name "tasia-agent[bot]"`
  await $`git config --global user.email "tasia-agent[bot]@users.noreply.github.com"`
}
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `tasia/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const shareUrl = shareId ? `[tasia session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
